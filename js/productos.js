    ...
    data.forEach(celular => {
      ...
      listItem.innerHTML = `
        <div class="celular-item">
          ...
          <button class="add-to-cart-btn" data-producto='${JSON.stringify(celular)}'>Añadir al Carrito</button>
        </div>`;
      celularesList.appendChild(listItem);
    });

    // Agrega un listener de eventos para cada botón "Añadir al Carrito"
...
function agregarAlCarrito(producto) {
    ...
    const existente = Array.from(lista.children).find((row) => {
        return row.querySelector(".borrar").getAttribute("data-id") === infoElemento.id;
    });
